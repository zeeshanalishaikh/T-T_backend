...
def apply_knn():
    ...
    cleaned_data = [item for item in data
                    if all(getattr(item, attr) is not None
    ...
    df = pd.DataFrame([
        (
            ...
            1 if str(item.classification) == 'ckd' else 0
                ...
                'hemo', 'pcv', 'wc', 'rc', 'rbc', 'pc', 'pcc', 'ba', 'htn', 'dm', 'cad', 'appet', 'pe', 'ane', 'classification'])
    ...
    categorical_columns = ['rbc', 'pc', 'pcc', 'ba', 'htn', 'dm', 'cad', 'appet', 'pe', 'ane']  # Update with your categorical columns
